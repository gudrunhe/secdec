...
# z*msq, expanding in z and then using z=1, here we use 3 Feynman parameters
# (i.e. we have three propagators)
...
propagators=("((k+p)**2)", "(k**2-msq_)", "(k**2-msq_)"),
loop_momenta=["k"],
regulators=['eps',],
replacement_rules = [
                        ('p*p', 'psq'),
...
propagators=li_m.propagators, loop_momenta=li_m.loop_momenta, regulators=li_m.regulators,
replacement_rules = [
                        ('p*p', 'psq'),
                        ('msq_', 'msq*z'),
                    ]
...
for name, real_parameters, smallness_parameter, li in (
    ("bubble1L_dotted_z_3fp", ['psq','msq','z'], "z", li_z),
    ("bubble1L_dotted_m_3fp", ['psq','msq'], "msq", li_m)
):
