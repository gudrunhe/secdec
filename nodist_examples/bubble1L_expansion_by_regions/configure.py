from pySecDec.amplitude_interface import Configuration, Qmc, CubaIntegrator, ContourDeformation
from os.path import isdir

for name in "bubble1L_expansion_by_regions_all_z", "bubble1L_expansion_by_regions_large_mass", \
            "bubble1L_expansion_by_regions_no_z", "bubble1L_expansion_by_regions_small_mass":
    if not isdir(name):
        continue
    # give the integral directory generated by `sum_pacakge`
    config = Configuration(name)

    # set options for the integrals
    config.set_options(
                        Qmc(transform="korobov3", fitfunction="polysingular"),
                        ContourDeformation(),
                    )

    # write the config_name.hpp file
    config.configure()