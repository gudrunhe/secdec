...
class LoopIntegral(object):
    ...
    of the following expressions that are accessible as member properties:
    ...
    * ``self.measure``,
    ...
    where ``self`` is an instance of either :class:`.LoopIntegralFromGraph`
    or :class:`.LoopIntegralFromPropagators`.

    Whereas ``self.numerator`` describes the numerator polynomial generated by tensor numerators or
    inverse propagators, ``self.measure`` contains the monomial associated with the integration
    measure in the case of propagator powers :math:`\neq 1`.
    The Gamma functions in the denominator belonging to the measure, however, are multiplied to the
    overall Gamma factor given by ``self.Gamma_factor``.
