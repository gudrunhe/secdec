...
#define INTEGRAL_NAME make_regions_ebr
...
TEST_CASE( "check result qmc", "[INTEGRAL_NAME]" ) {
    ...
    SECTION("default integral transform") {
        ...
        const std::vector<INTEGRAL_NAME::real_t> real_parameters = { 0.01 };
        ...
        std::vector<INTEGRAL_NAME::nested_series_t<INTEGRAL_NAME::sum_t>> unwrapped_amplitudes =
            INTEGRAL_NAME::make_amplitudes(real_parameters, complex_parameters, "../make_regions_ebr/make_regions_ebr_coefficients", integrator);
        ...
        (
            ...
            {
                 0.0                     + 0.0 * I, // eps ** -1
                 4.60517018598809091e+00 + 0.0 * I, // eps **  0 // obtained from high precision run of pysecdec
            },

            true, // series is truncated above; i.e. "+ O(eps**2)"
            "delta" // the expansion parameter
        );
