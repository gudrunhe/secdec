...
class CheckQmcErrorMessages(unittest.TestCase):
    def setUp(self):
        # load c++ library
        self.lib = IntegralLibrary('../no_default_transform/no_default_transform_pylink.so')

    def test_setting_errormode(self):
        self.assertRaisesRegex(ValueError, 'Unknown `errormode` "foo"', self.lib.use_Qmc, transform='korobov2x1', errormode='foo')

        # test known errormodes
        self.lib.use_Qmc(errormode='default', transform='korobov2x1')
        self.lib.use_Qmc(errormode='all', transform='korobov2x1')
        self.lib.use_Qmc(errormode='largest', transform='korobov2x1')
...
class CheckLib(unittest.TestCase):
    ...
    def check_result(self, computed_series, epsrel, epsabs):
        ...
        for order in range(-1,2):
            ...
            self.assertAlmostEqual(  value.real, self.target_result[order].real, delta=3.*epsabs  )
            self.assertAlmostEqual(  value.imag, self.target_result[order].imag, delta=3.*epsabs  )
    ...
    def test_CQuad(self):
        # choose integrator
        self.lib.use_CQuad(verbose=False, epsrel=self.epsrel, epsabs=self.epsabs) # set up CQuad/MultiIntegrator
